<x-app-layout>
    ...
    <x-slot name="header">
        <div class="text-base md:text-2xl">社員情報 編集</div>
    </x-slot>
...
@endif
                <div class="my-2 px-2 py-2 rounded-lg border-gray-300 border-1 bg-white">
                    <form name="input" class="mx-auto needs-validation" novalidate method="POST" action="{{ route('employee.edit-confirm') }}">
@csrf
                        <!-- ユーザID -->
                        <input type="hidden" id="user_id" name="user_id" value="{{ empty(old('user_id')) ? $user->id : old('user_id') }}">
                        ...
                        <div class="col-sm mb-2">
                            ...
                            <div class="border border-1 border-gray-500 rounded-lg p-2">
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" id="user_authority_manager" name="user_authority" value="1" @if ((empty(old('user_authority')) && ($user->user_authority == 1)) || old('user_authority') == 1) checked @endif>
                                    <label class="form-check-label" for="user_authority_manager">管理者</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" id="user_authority_employee" name="user_authority" value="2" @if ((empty(old('user_authority')) && ($user->user_authority == 2)) || old('user_authority') == 2) checked @endif>
                        ...
                        <!-- メールアドレス -->
                        <div class="col-sm mb-2">
                            <label for="email" class="col-form-label">メールアドレス<span class="badge bg-secondary mx-2 my-0">任意</span><br><span class="col-form-label-sm text-primary"><i class="fas fa-info-circle"></i>&nbsp;未設定の場合、メールによるパスワードのリセット機能は利用出来ません</span></label>
                        ...
                        <div class="col-sm mb-2">
                            <label for="password" class="col-form-label">パスワード
                                <span class="badge bg-secondary mx-2 my-0">任意</span><br>
                                <span class="col-form-label-sm text-danger"><i class="fas fa-exclamation-circle"></i>&nbsp;変更する場合のみ入力してください</span>
                            </label>
                        ...
                        <!-- ユーザ状態 -->
                        <div class="col-sm mb-2">
                            <label for="user_status" class="col-form-label">ユーザ状態<span class="badge bg-danger mx-2 my-0">必須</span><br><span class="col-form-label-sm text-primary"><i class="fas fa-info-circle"></i>&nbsp;無効で登録した場合、有効に更新するまでユーザはログイン出来ません</span></label>
                            <div class="border border-1 border-gray-500 rounded-lg p-2">
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" id="user_status_on" name="user_status" value="1" @if ($user->user_status == 1 || empty(old('user_status')) || old('user_status') == 1) checked @endif>
                                    <label class="form-check-label" for="user_status_on">有効</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" id="user_status_off" name="user_status" value="0" @if ($user->user_status == 0 || (!empty(old('user_status')) && old('user_status') == 0)) checked @endif>
                        ...
                        <div class="mx-auto my-2 text-center">
                            <a class="btn btn-lg btn-primary mx-4 my-2" href="{{ route('employee.list') }}">戻&emsp;る</a>
